...
def image_points_to_ray(image_points, camera_mat, world_mat):
    ...
    #Ajouter par ly projection sur la camera
    #mat_inv = to_pytorch(np.linalg.inv(world_mat[:,:3,:3]))
    #inv_point = torch.einsum('bij,bnj->bni',mat_inv, ray_dirs)
    #inv_point = inv_point[:,:,:2]
    #inv_point = inv_point * camera_mat[0,0,0] + camera_mat[0,:2,2]
    #inv_point = (torch.round(inv_point)).int()
